...
Add the custom behavioral RTL files from the `hdl/` folder into your Vivado project as design sources:
- `hdl/fp16_fft16_top.v` (Top-level FFT controller module)
- `hdl/fp16_butterfly.v` (Combinational butterfly calculating unit)
- `hdl/fp16_add.v` (Custom FP16 adder)
- `hdl/fp16_sub.v` (Custom FP16 subtractor)
- `hdl/fp16_mul.v` (Custom FP16 multiplier)
- `hdl/fp16_twiddle_rom.v` (Pre-computed FP16 Twiddle factors)
- `hdl/sample_ram.v` (Standard double-port RAM)
...
2. Add the Verilog files listed above and the constraints file `constraints/fft16_constraints.xdc`.
...
iverilog -o sim/test_sim.vvp sim/tb_fft16.v hdl/fp16_fft16_top.v hdl/sample_ram.v hdl/fp16_twiddle_rom.v hdl/fp16_butterfly.v hdl/fp16_add.v hdl/fp16_sub.v hdl/fp16_mul.v

# Run simulation using vvp to generate sim_1024_fp16.log in the sim/ directory
