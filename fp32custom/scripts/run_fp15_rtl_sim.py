...
backup_dir = os.path.join(project_root, "backups", "backup_fp12")
if not os.path.exists(backup_dir):
    os.makedirs(backup_dir)
    files_to_backup = [
        ("hdl/fp16_add.v", "fp16_add.v"),
        ("hdl/fp16_sub.v", "fp16_sub.v"),
        ("hdl/fp16_mul.v", "fp16_mul.v"),
        ("hdl/fp16_butterfly.v", "fp16_butterfly.v"),
        ("hdl/fp16_fft16_top.v", "fp16_fft16_top.v"),
        ("sim/tb_fft16.v", "tb_fft16.v"),
        ("hdl/fp16_twiddle_rom.v", "fp16_twiddle_rom.v")
    ]
    for src_rel, file_name in files_to_backup:
        src = os.path.join(project_root, src_rel)
        dst = os.path.join(backup_dir, file_name)
        if os.path.exists(src):
            with open(src, 'rb') as sf, open(dst, 'wb') as df:
                df.write(sf.read())
            print(f"Backed up {src_rel} to backups/backup_fp12/{file_name}")
...
with open('hdl/fp16_twiddle_rom.v', 'w', encoding='utf-8') as f:
    f.write('\n'.join(twiddle_lines) + '\n')
print("Generated FP15 twiddle ROM in hdl/fp16_twiddle_rom.v")
...
print("Updated hdl/fp16_add.v with FP15 design")
...
"""
with open('hdl/fp16_sub.v', 'w', encoding='utf-8') as f:
    f.write(fp15_sub_code)
print("Updated hdl/fp16_sub.v with FP15 design")
...
"""
with open('hdl/fp16_mul.v', 'w', encoding='utf-8') as f:
    f.write(fp15_mul_code)
print("Updated hdl/fp16_mul.v with FP15 design")
...
"""
with open('hdl/fp16_butterfly.v', 'w', encoding='utf-8') as f:
    f.write(fp15_butterfly_code)
print("Updated hdl/fp16_butterfly.v with FP15 design")

# 9. Modify fp16_fft16_top.v to change default N=15
with open('hdl/fp16_fft16_top.v', 'r', encoding='utf-8') as f:
    top_code = f.read()
top_code = re.sub(r'parameter N\s*=\s*\d+', 'parameter N          = 15', top_code)
with open('hdl/fp16_fft16_top.v', 'w', encoding='utf-8') as f:
    f.write(top_code)
print("Updated hdl/fp16_fft16_top.v default parameter N = 15")
...
tb_code = re.sub(r'\bfft16_top\b', 'fp16_fft16_top', tb_code)
...
cmd_compile = "iverilog -o sim/sim.out hdl/fp16_add.v hdl/fp16_sub.v hdl/fp16_mul.v hdl/fp16_butterfly.v hdl/fp16_twiddle_rom.v hdl/sample_ram.v hdl/fp16_fft16_top.v sim/tb_fft16.v"
