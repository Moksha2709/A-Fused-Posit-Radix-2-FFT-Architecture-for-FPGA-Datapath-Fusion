...
always @(posedge clk) begin
    if (state == WAIT2) begin
        ...
        $display("  INPUTS : A = %h + j%h   B = %h + j%h",
                 ar, ai, br, bi);

        $display("  TWIDDLE: W = %h + j%h",
                 wr, wi);
    end
end
...
always @(posedge clk) begin
    if (state == COMPUTE) begin
        $display("  OUTPUTS: Y0 = %h + j%h   Y1 = %h + j%h",
                 y0r, y0i, y1r, y1i);
    end
end
// synthesis translate_on
...
always @(posedge clk) begin
    if (state == WRITE) begin
        $display("  WRITE  : RAM[%0d] <= %h + j%h",
                 addr_a_r, y0r_r, y0i_r);

        $display("           RAM[%0d] <= %h + j%h",
