...
# Detect if using Scenario B
is_scenario_b = "fmcw_dual_target_scenario_B.txt" in tb_text

if is_scenario_b:
    # Find weak peak in range 25 to 50
    mat_peak_idx = int(np.argmax(mat_mags[25:50])) + 25
    rtl_peak_idx = int(np.argmax(rtl_mags[25:50])) + 25
else:
...
if is_scenario_b:
    ...
    true_range = mat_interp * range_coeff
    est_range = rtl_interp * range_coeff
else:
