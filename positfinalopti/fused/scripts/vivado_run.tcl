...
set PROJ_NAME "fused_posit_fft_N16"
...
# --------------------------------------------------------------------------- #
add_files [list \
    $SRC_DIR/../rtl/fft16_top_fused.v         \
    $SRC_DIR/../rtl/sample_ram.v              \
    $SRC_DIR/../rtl/twiddle_rom.v             \
    $SRC_DIR/../rtl/posit_fused_butterfly.v   \
    $SRC_DIR/../rtl/posit_addsub_raw.v        \
    $SRC_DIR/../rtl/posit_mul_raw.v           \
    $SRC_DIR/../rtl/POSIT_decode.v            \
    $SRC_DIR/../rtl/POSIT_encode.v            \
    $SRC_DIR/../rtl/posit_raw_to_enc.v        \
]

# --------------------------------------------------------------------------- #
# 3. Add Constraints
# --------------------------------------------------------------------------- #
add_files -fileset constrs_1 $SRC_DIR/../rtl/fft16_constraints.xdc

# --------------------------------------------------------------------------- #
# 4. Set Top Module and Parameters (N=16, POINTS=1024)
# --------------------------------------------------------------------------- #
set_property top fft16_top_fused [current_fileset]
set_property generic {N=16 POINTS=1024 LOG2_POINTS=10} [current_fileset]
update_compile_order -fileset sources_1

puts "=== Project created. Top = fft16_top_fused, N=16, POINTS=1024 ==="
...
puts ""
puts "============================================================"
puts " FUSED POSIT(16,1) FFT SYNTHESIS SUMMARY"
puts "============================================================"
